...
#include <gps/Position.h>
#include <gps/Status.h>
...
// class vn300_node
// Description: interfaces with vectornav sensor via vnproglib and outputs ros messages
...
class vn300_node {
	...
	nbsdx::concurrent::ThreadPool<5> workers;
	...
	public:
		...
		void publish_heading_wrapper(gps::Heading msg) { heading.publish(msg); };
		void publish_position_wrapper(gps::Position msg) { position.publish(msg); };
		void publish_status_wrapper(gps::Status msg) { status.publish(msg); };

		unsigned published() { return workers.JobsRemaining(); };
};
...
void vn300_packet_handler(void *userdata, vn::protocol::uart::Packet &p, size_t index) {
	...
	vn300_node *obj = (vn300_node *)userdata;
	...
	rate(115200)
{
	...
	// params
	node.param("device", device, device);
	node.param("rate", rate, rate);
	
	// setup
	
	try {
		sensor.connect(device, rate);
	} catch(vn::not_found &e) {}

	ROS_INFO("Connected to %s at %d baud", device.c_str(), rate);

	heading = node.advertise< gps::Heading >("heading", true);
	position = node.advertise< gps::Position >("position", true);
	status = node.advertise< gps::Status >("status", true);
...
int main(int argc, char **argv) {
	...
	while(ros::ok() && gps.ok()) { 
		ROS_INFO("Publishing jobs remaining: %d", gps.published());
		r.sleep(); 
	}
